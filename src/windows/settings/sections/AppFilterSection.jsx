...
function AppFilterSection({
  ...
  useEffect(() => {
    if (settings.appFilterList && Array.isArray(settings.appFilterList)) {
      ...
      const list = settings.appFilterList.split('\n').map(item => item.trim()).filter(item => item.length > 0);
      setAppList(list);
    }
      ...
      setAppIconMap(prevIconMap => {
        ...
        appList.forEach(appName => {
          if (!updatedIconMap.has(appName)) {
            const matchedApp = uniqueApps.find(app => app.process.toLowerCase() === appName.toLowerCase() || app.process.toLowerCase().includes(appName.toLowerCase()) || appName.toLowerCase().includes(app.process.toLowerCase()));
      ...
      <SettingsSection title={t('settings.appFilter.manageTitle')} description={t('settings.appFilter.manageDesc')}>
        <SettingItem label={t('settings.appFilter.addApp')} description={t('settings.appFilter.addAppDesc')}>
          <div className="flex items-center gap-2">
            <Input value={customAppInput} onChange={e => setCustomAppInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddCustomApp()} placeholder={t('settings.appFilter.inputPlaceholder')} className="flex-1" />
